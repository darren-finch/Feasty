...
class MealPlanController(
...
) : BaseController, MealPlanViewMvc.Listener, DialogsEventBus.Listener {
    ...
    fun getAllMealPlans() {
        ...
        getAllMealPlansJob = CoroutineScope(backgroundContext).launch {
            ...
            viewModel.setMealPlans(allMealPlans)
    ...
    private fun refreshMealsForSelectedMealPlan() {
        if (canRefreshMeals()) {
            getMealsForMealPlanJob = CoroutineScope(backgroundContext).launch {
                ...
                withContext(uiContext) {
                    viewMvc.hideProgressIndication()
                    if (mealPlanMeals.isEmpty()) viewMvc.showEmptyListIndication()
    ...
    override fun onDeleteMealPlanMealClicked(mealPlanMeal: UiMealPlanMeal) {
        deleteMealPlanMealJob = CoroutineScope(backgroundContext).launch {
            deleteMealPlanMealUseCase.deleteMealPlanMeal(mealPlanMeal.id)
            refreshMealsForSelectedMealPlan()
        }
    }
